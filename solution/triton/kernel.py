...
def _gdn_decode_kernel(
    ...
    stride_sb, stride_sh, stride_sd, stride_sk,
    ...
    k_vec = tl.load(k_ptr + b_idx * stride_kb + qh_idx * stride_kh + kd * stride_kd).to(tl.float32) * scale
    q_vec = tl.load(q_ptr + b_idx * stride_qb + qh_idx * stride_qh + kd * stride_qd).to(tl.float32)

    rows   = tile_idx * BLOCK_ROWS + tl.arange(0, BLOCK_ROWS)
    v_tile = tl.load(v_ptr + b_idx * stride_vb + vh_idx * stride_vh + rows * stride_vd).to(tl.float32)

    s_base     = state_ptr + b_idx * stride_sb + vh_idx * stride_sh
    state_tile = tl.load(s_base + rows[:, None] * stride_sd + kd[None, :] * stride_sk).to(tl.float32)
    ...
    tl.store(ns_base + rows[:, None] * stride_sd + kd[None, :] * stride_sk, new_state.to(tl.float32))
...
def kernel(
    ...
    sc = float(scale.item()) if isinstance(scale, torch.Tensor) else (1.0 / math.sqrt(D) if not scale else float(scale))
    ...
    total = B * Hv
    if   total >= 592: BLOCK_ROWS, NW, NS = 128, 4, 3
    elif total >= 296: BLOCK_ROWS, NW, NS =  64, 2, 3
    elif total >= 148: BLOCK_ROWS, NW, NS =  32, 1, 4
    else:              BLOCK_ROWS, NW, NS =  16, 1, 4
